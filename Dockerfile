...
RUN apt-key adv --no-tty --keyserver http://keyserver.ubuntu.com:80 --recv-keys 3FA7E0328081BFF6A14DA29AA6A19B38D3D831EF 
...
# cURL, Varnish Redis tools and parallel
RUN apt-get update && apt-get install -y curl varnish redis-tools parallel

# S3cmd
RUN wget https://github.com/s3tools/s3cmd/archive/master.zip && unzip master.zip && rm master.zip && cd s3cmd-master && python setup.py install
